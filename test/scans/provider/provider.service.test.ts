...
describe('ProviderService', () => {
  it('should be defined', () => {
    ...
    const service = new ProviderService(mockModel as any);
  ...
  it('should call findOne with correct slug', async () => {
    const mockModel = { findOne: jest.fn().mockResolvedValue({ id: 1 }), findAndCountAll: jest.fn() };
    const service = new ProviderService(mockModel as any);
    const result = await service.getBySlug('slug-123');
    expect(mockModel.findOne).toHaveBeenCalledWith({ where: { slug: 'slug-123' } });
